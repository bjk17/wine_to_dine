...
class TestRedWineRecommendations(unittest.TestCase):
    ...
    def test_recommendation_sunny(self) -> None:
        recommendations = list(assign_scorings([self.inventory_item],
                                               [self.scoring]))
        self.assertEqual(1, len(recommendations))
        inventory_item, certainty, scoring = recommendations[0]
        self.assertEqual(self.inventory_item, inventory_item)
